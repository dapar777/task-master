...
- ⏳ **Čeká do… (odklad s odpočtem)** – přechodem na tento stav zvolíš v dialogu interval **posuvníky na dny, hodiny a minuty** (předvyplní se **naposledy použitá** hodnota, poprvé **10 min** z `DEFAULT_SNOOZE`). Na kartě i ve stromu běží **odpočet**; jakmile vyprší, ukáže se **⏰ Čas vypršel** a tlačítko **↻ Obnovit**, které úkol odloží znovu o **stejný interval**. Odklad má **barvu čekajícího úkolu** a po doběhnutí si ji ponechá – že vypršel, dává najevo pozice nahoře a text, ne změna barvy.
  ...
  - **Vrátit stav**: odklad si pamatuje, odkud se odkládalo (`_snooze_prev`), takže po vypršení jde jedním krokem zpět – na kartě vedle *Obnovit* i v paletě (*Úkol › Vrátit stav před odložením*).
...
- 🔗 **Sekvence úkolů** (`Ctrl+Shift+R`, menu *Úkol* i kontextové menu) – z **označených úkolů** udělá řetěz: druhý čeká na první, třetí na druhý atd. Před provedením se otevře **dialog**, kde pořadí zkontroluješ, přeskládáš (▲▼ nebo tažením) nebo položku vyřadíš (✕). První úkol zůstane volný a **dokončením se vždy odemkne jen následující**. Vrátit lze jedním `Ctrl+Z`.
...
- 🤖 **Automatické blokování** – jakmile má úkol aspoň jeden nedokončený **přímý** podúkol a **všechny** jeho nedokončené přímé podúkoly jsou ve stavu *Čeká*, *Čeká do…* (s běžícím odpočtem) nebo *Blokováno*, úkol se sám přepne na *Blokováno* (značka „⛔ auto"). **Doběhlý odklad rodiče neblokuje** – už nečeká, volá po akci; naopak sám auto-blokaci **podléhá** (úkol s vypršelým intervalem a jen čekajícími podúkoly se zablokuje jako každý jiný). Jakmile podmínka pomine (některý podúkol se rozpracuje/dokončí), vrátí se automaticky na *Ke zpracování*. **Ruční** blokování se nepřepisuje.
...
- 📇 **Skupiny podle stavu v Bez rušení** – karty se řadí do pevného pořadí skupin: **doběhlé odklady → Probíhá + Ke zpracování → Čeká (i běžící odklady) → Blokováno → Hotovo**. Přesun v pořadí (`Ctrl+W/Q`) se pohybuje **jen v rámci skupiny**.
