...
print("8) Konzistence i po kopírování a vyjmutí")
win.workspace.create_root("Zdroj")
win.workspace.create_root("Cilovy2")
win.workspace.load()
win._populate()
app.processEvents()
win._current_node = node("Zdroj")
win._copy_task()
win._current_node = node("Cilovy2")
win._paste_task()
app.processEvents()
consistent("po kopii + vložení")
win._current_node = node("Zdroj")
win._cut_task()
win._current_node = node("Cilovy2")
win._paste_task()
app.processEvents()
consistent("po vyjmutí + vložení")
win._undo()
app.processEvents()
consistent("po undo vyjmutí")
